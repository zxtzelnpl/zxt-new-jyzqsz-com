...
(function ($) {
  ...
  function DropDown($dom, options) {
    ...
    this._selectedHandleClick = this._selectedHandleClick.bind(this);
    this._boxHandleClick = this._boxHandleClick.bind(this);
    this.init();
  }
  ...
  DropDown.prototype = {
    init: function () {
      var me = this;
      me.box.find('.drop-down-option').each(function (index, option) {
        var $option = $(option);
        if ($option.data('select') === 'select') {
          me.selected.data('value', $option.data('value'));
          me.selected.html($option.html())
        }
      });

      this.selected.on('click', this._selectedHandleClick);
      this.box.on('click', this._boxHandleClick);
    },
    ...
    _boxHandleClick: function (e) {
      ...
      if ($(p).hasClass('drop-down-option')) {
        ...
        this.options.select(p, value, html);
